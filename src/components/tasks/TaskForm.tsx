...
export const TaskForm: React.FC<TaskFormProps> = ({ projectId, initialData, onSubmit, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!epicId) newErrors.epicId = 'Epic is required';
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <FormField label="Epic" required error={errors.epicId}>
        <Select 
          value={epicId} 
          onChange={(e) => { setEpicId(e.target.value); setErrors(prev => ({...prev, epicId: ''})); }}
          options={[{ value: '', label: 'Select an epic' }, ...epicOptions]}
        />
      </FormField>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <FormField label="Status">
          <Select 
            ...
            options={(activeProject?.workflowStatuses || []).map((s) => ({ value: s.id, label: s.label }))}
          />
        </FormField>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1rem' }}>
        ...
        <Button type="submit" variant="primary" loading={isSubmitting}>Save Task</Button>
      </div>
    </form>
  );
};
